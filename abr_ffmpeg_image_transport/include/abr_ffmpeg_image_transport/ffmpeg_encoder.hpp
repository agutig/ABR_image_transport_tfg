...
#include "abr_ffmpeg_image_transport/tdiff.hpp"
...
public:
  ...
  void setLevel(const std::string & p)
  {
    Lock lock(mutex_);
    level_ = p;
  }
  ...
  std::string level_;       // e.g. "tune"
  int qmax_{0};            // max allowed quantization. The lower the better quality
  int GOPSize_{0};        // distance between two keyframes
...
private:
  ...
  mutable std::recursive_mutex drainMutex_;
